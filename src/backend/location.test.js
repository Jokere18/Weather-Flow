...
it("should successfully fetch location and weather data for all US states", async () => {
    ...
    for (const state of US_STATES) {
        try {
            ...
            const locationInfo = await (0, location_1.fetchLocationData)(axios_1.default, GEOCODE_API_URL, state);
            const weatherInfo = await (0, weatherapi_1.fetchWeatherData)(axios_1.default, WEATHER_API_URL, locationInfo.lat, locationInfo.lon);
            results.push({
                state,
