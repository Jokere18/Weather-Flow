...
it("should convert API response", async () => {
    ...
    const result = await (0, weatherapi_1.fetchWeatherData)(axios_1.default, WEATHER_API_URL, "0.0", "0.0");
    //assert.ok(result instanceof CurrentWeather);
});
it("throws error when response is not 200", async () => {
    ...
    await expect((0, weatherapi_1.fetchWeatherData)(axios_1.default, WEATHER_API_URL, "0.0", "0.0")).rejects.toThrow();
});
...
it("gets low temp", () => {
    const response = weatherapi_1.currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
    const currentWeather = new weatherapi_1.CurrentWeather(response);
    const lowTemp = currentWeather.lowTemp();
    assert_1.strict.strictEqual(lowTemp, 23.3);
});
it("gets high temp", () => {
    const response = weatherapi_1.currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
    const currentWeather = new weatherapi_1.CurrentWeather(response);
    const highTemp = currentWeather.highTemp();
    assert_1.strict.strictEqual(highTemp, 33.5);
});
it("gets condition", () => {
    const response = weatherapi_1.currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
    const currentWeather = new weatherapi_1.CurrentWeather(response);
    const condition = currentWeather.condition();
    assert_1.strict.strictEqual(condition, "Clear sky");
});
