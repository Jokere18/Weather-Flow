...
it("should convert API response", async () => {
  ...
  const result = await fetchWeatherData(axios, WEATHER_API_URL, "0.0", "0.0");
...
it ("gets low temp", () => {
  ...
  assert.strictEqual(lowTemp, 23.3);
})
...
it ("gets high temp", () => {
  ...
  assert.strictEqual(highTemp, 33.5);
})  
...
it ("gets condition", () => {
  ...
  assert.strictEqual(condition, "Clear sky");
})